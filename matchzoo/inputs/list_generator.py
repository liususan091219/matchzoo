...
class ListBasicGenerator_linear(object):
    def __init__(self, data_root, config={}):
        self.__name = 'ListBasicGenerator_linear'
        self.config = config
        self.batch_list = config['batch_list']
        if 'relation_file' in config:
            self.rel = read_relation_linear(filename= data_root + config['relation_file'])
            self.list_list = self.make_list_linear(self.rel)
            self.num_list = len(self.list_list)
        self.check_list = []
        self.point = 0
    ...
    def make_list_linear(self, rel):
        list_list = {}
        for label, d1, d2, d3, d4 in rel:
            if d1 not in list_list:
                list_list[d1] = []
            list_list[d1].append( (label, d2, d3, d4) )
        for d1 in list_list:
            list_list[d1] = sorted(list_list[d1], reverse = True)
        print('List Instance Count:', len(list_list), end='\n')
        return list(list_list.items())
...
class DRMM_ListGenerator_linear(ListBasicGenerator_linear):
    def __init__(self, data_root, config={}):
        super(DRMM_ListGenerator_linear, self).__init__(data_root, config=config)
        self.data1 = config["data1"]
        self.data2 = config["data2"]
        ...
        if 'hist_feats_file_title' in config:
        ...
        if not self.check():
            raise TypeError('[DRMM_ListGenerator_linear] parameter check wrong.')
        print('[DRMM_ListGenerator_linear] init done, list number: %d. ' % (self.num_list), end='\n')
    ...
    def cal_hist(self, t1, t2, thisdata2, thishistfeats_variable, data1_maxlen, hist_size):
        ...
        if self.use_hist_feats:
            ...
            caled_hist = np.reshape(thishistfeats_variable[(t1, t2)], (d1len, hist_size))
    ...
    def get_all_data(self):
        x1_ls, x1_len_ls, x2_ls, x2_len_ls, x3_ls, x3_len_ls, x4_ls, x4_len_ls, y_ls, list_count_ls = [], [], [], [], [], [], [], [], [], []
        while self.point < self.num_list:
            currbatch = []
            if self.point + self.batch_list <= self.num_list:
                currbatch = self.list_list[self.point: self.point + self.batch_list]
            ...
            x2_len_ls.append(X2_len)
            x3_ls.append(X3)
            x3_len_ls.append(X3_len)
            x4_ls.append(X4)
            x4_len_ls.append(X4_len)
            y_ls.append(Y)
            list_count_ls.append(list_count)
        return x1_ls, x1_len_ls, x2_ls, x2_len_ls, x3_ls, x3_len_ls, x4_ls, x4_len_ls, y_ls, list_count_ls
