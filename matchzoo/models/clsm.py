...
class CLSM(BasicModel):
    def __init__(self, config):
        super(CLSM, self).__init__(config)
        self.__name = 'CLSM'
    ...
    def build(self):
        ...
        conv1d_Q = Convolution1D(self.config['kernel_count'], self.config['kernel_size'], padding='same', activation='relu')
        conv1d_D = Convolution1D(self.config['kernel_count'], self.config['kernel_size'], padding='same', activation='relu')
        q_conv = conv1d_D(q_embed)
        show_layer_info('Convolution1D', q_conv)
        q_conv = Dropout(self.config['dropout_rate'])(q_conv)
        show_layer_info('Dropout', q_conv)
        d_conv = conv1d_D(d_embed)
        ...
        mlp_Q = mlp_work(self.config['kernel_count'])
        mlp_D = mlp_work(self.config['kernel_count'])
        ...
        #smoothing 
        gamma = K.variable(1.0, name='gamma')
        out_ = Lambda(lambda x: gamma*x)(out_)
