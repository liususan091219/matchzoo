...
def train(config, data_root, log_file):
    ...
    for tag in input_conf:
        ...
        if 'text2_corpus' in input_conf[tag]:
            ...
            if datapath not in dataset:
                dataset[datapath], _ = read_data(datapath)
        if 'text3_corpus' in input_conf[tag]:
            datapath = data_root + input_conf[tag]['text3_corpus']
            if datapath not in dataset:
                dataset[datapath], _ = read_data(datapath)
        if 'text4_corpus' in input_conf[tag]:
            datapath = data_root + input_conf[tag]['text4_corpus']
            if datapath not in dataset:
                dataset[datapath], _ = read_data(datapath)
    ...
    for tag, conf in input_train_conf.items():
        ...
        conf['data3'] = dataset[data_root + conf['text3_corpus']]
        conf['data4'] = dataset[data_root + conf['text4_corpus']]
